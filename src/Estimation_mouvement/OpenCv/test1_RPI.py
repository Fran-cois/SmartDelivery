...
cap = cv2.VideoCapture(1)                       # a changer eventellement 
...
while(cap.isOpened()):
    ...
    if (lines is None):
        ...
        if (np.size(tab1)==0 or np.size(tab2)==0):
            ...
            cv2.putText(small, 'decalage = ' + str(decalagecm) + " cm" + "," + str(thetamdeg-90)+ " deg", (10, 30), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 1, cv2.CV_AA)
